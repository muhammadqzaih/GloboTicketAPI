...
{
    public class GetOrdersForMonthQueryHandler : 
...

    {
        ...
        public GetOrdersForMonthQueryHandler(IOrderRepository oderRepository, IMapper mapper)
        {
           ...
           GetOrdersForMonthQuery request, CancellationToken cancellationToken)
        {
            var list = await _orderRepository
                .GetPagedOrdersForMonth(request.Date, request.Page, request.Size);

            var orders = _mapper.Map<List<OrdersForMonthDto>>(list);

            var count = await _orderRepository
                .GetTotalCountOfOrdersForMonth(request.Date);
